...
with open('../model/classification/label_encoder.json', 'w') as f:
...
model = Models.create_classification_model(input_dim=input_dim, num_classes=num_classes)
...
checkpoint = ModelCheckpoint('../model/classification/trained_classification_model_{epoch:02d}_{val_loss:.4f}', save_best_only=True)
...
# Save statistics
with open('../model/classification/statistics.pkl', 'wb') as f:
...
model.save('../model/classification/trained_classification_model')
...
with open('../model/classification/metadata.json', 'w') as f:
    f.write(metadata_json)

statistics_json = json.dumps(train_statistics)

with open('../model/classification/statistics.json', 'w') as f:
...
coreml_model.save('../model/classification/classification.mlmodel')

# ** Evaluate on the test set **
print("*** Evaluating model ***")
# Load the labels
loaded_classes = np.load('../model/classification/label_encoder.npy')
label_encoder = LabelEncoder()
...
with open('../model/classification/classification-lite.tflite', 'wb') as f:
...
X_test = X_test.reshape((X_test.shape[0], X_test.shape[1], 1)) # Assuming that the classification expects a 3D input. Adjust if otherwise.
